...
public class Gun : MonoBehaviour
{
    ...
    public void ShootGun(Transform director)
    {
        ...
        if (!shotgun) // Single mode
        {
            SpawnBullet(director.position, director.rotation);
        }
        else // Shotgun mode
        {
            float halfSpread = shotgunSpread / 2f;
            float angleStep = shotgunSpread / (shotgunBullets - 1);
            
            // Calculate starting angle to center the spread
            float currentAngle = -halfSpread;
            
            // Spawn the specified number of bullets
            for (int i = 0; i < shotgunBullets; i++)
            {
                // Create rotation for each bullet by adding spread angle to gun's rotation
                Quaternion bulletRotation = director.rotation * Quaternion.Euler(0f, 0f, currentAngle);
                SpawnBullet(director.position, bulletRotation);
                
                // Move to next spread angle
